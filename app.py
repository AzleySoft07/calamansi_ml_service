...
def _prepare_history(
    ...
    if not required.issubset(history.columns):
        raise ValueError(
            f"history must contain: {', '.join(sorted(required))}"
        )
    ...
    stats = {
        str(row.get("barangay", "")).strip().upper(): {
            "trees": max(
                0.0,
                float(row.get("trees", 0) or 0),
            ),
            "farmers": max(
                0.0,
                float(row.get("farmers", 0) or 0),
            ),
        }
    ...
    if grouped.empty:
        raise ValueError(
            "history contains no valid observations"
        )
    ...
    names = sorted(
        set(stats) | set(grouped["barangay"])
    )
...
def _predict_month(
    ...
    month: int,
    barangays: list[dict[str, Any]],
    history_by_name: dict[str, list[float]],
) -> dict[str, float]:
    rows = []
    names = []
    ...
    prediction_frame = pd.DataFrame(rows)[FEATURES]
...
def forecast():
    ...
    try:
        forecast_year = int(
            payload.get("forecast_year") or 0
        )

        five_years = [
            int(year)
            for year in (
                payload.get("five_years")
                or []
            )
        ]
        ...
        return _error(
            "forecast_year and five_years "
            "must contain valid years."
        )

    if not barangays or forecast_year < 2000:
        return _error(
            "barangays and a valid "
            "forecast_year are required"
        )
    ...
    if (
        len(frame) < 24
        or frame["quantity"].gt(0).sum() < 12
    ):
        return _error(
            "Not enough historical "
            "observations to train Random "
            "Forest reliably.",
            422,
        )

    # Validate using the newest 20%
    # of chronological observations.
    split = max(
        1,
        int(len(frame) * 0.8),
    )
    ...
    # Train again using all available
    # historical records.
    model.fit(
        frame[FEATURES],
        frame["quantity"],
    ...
    annual: dict[str, float] = defaultdict(
        float
    )

    monthly_target = [0.0] * 12

    barangay_target: dict[
        str,
        float,
    ] = defaultdict(float)
